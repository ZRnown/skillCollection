...
export function findChromeExecutable(): string | undefined {
  const override = process.env.QIEHAO_BROWSER_CHROME_PATH?.trim();
...
export function getDefaultProfileDir(): string {
  ...
  return path.join(base, 'qiehao-browser-profile');
}
...
export async function launchChrome(url: string, profileDir?: string): Promise<{ cdp: CdpConnection; chrome: ReturnType<typeof spawn>; port: number; profileDir: string }> {
  const chromePath = findChromeExecutable();
  if (!chromePath) throw new Error('Chrome not found. Set QIEHAO_BROWSER_CHROME_PATH if needed.');

  const profile = profileDir ?? getDefaultProfileDir();
  await mkdir(profile, { recursive: true });

  const port = await getFreePort();
  console.error(`[cdp] Launching Chrome (profile: ${profile}, port: ${port})`);
  ...
  return { cdp, chrome, port, profileDir: profile };
}
